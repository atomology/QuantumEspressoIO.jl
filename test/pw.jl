...
@testitem "read_hubbard!" begin
    ...
    expected = OrderedDict{String,Any}(
        "option" => "atomic",
        "types" => ["U", "V"],
        "manifolds" => ["Ni-3d", "Ni-3d O-2p 1 12"],
        "values" => [5.77, 3.5],
    )
    @test content == expected
    @test lcopy == ["following line"]
...
@testitem "read_hubbard_dat" begin
    ...
    params = QuantumEspressoIO.read_hubbard_dat(io)
    @test params["option"] == "atomic"
    @test params["types"] == ["U", "V"]
    @test params["manifolds"] == ["Ni-3d", "Ni-3d O-2p 1 12"]
    @test params["values"] == [5.77, 3.5]
end
...
@testitem "write_hubbard" begin
    ...
    buf = IOBuffer()
    QuantumEspressoIO.write_hubbard(buf, card)
    out = String(take!(buf))
