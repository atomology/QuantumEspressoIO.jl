...
```
"""
function read_hubbard!(lines::AbstractVector, n_species::Integer)
    # TODO
    name = "hubbard"
    icard = find_card(lines, name)
    ...
    nline = num_valid_lines(lines, icard, iend) - 1
    ...
    card[:values] = map(x -> parse(Float64, split(x)[end]), content[2])
    return Symbol(name) => card
end
...
function read_pw_in(io::Union{IO,AbstractString})
    ...
    card = read_hubbard!(cards, n_species)
    isnothing(card) || push!(params, card)
